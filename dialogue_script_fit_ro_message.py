...
def fit_ro_message(user_id, message, tag):
    ...
    if tag == 'birth_date':
        ...
        lead_fit_ro_mess = get_lead_fit_ro_mess_info(user_id)
        response = handle_answer_question(user_id, message, lead_fit_ro_mess['age_w_h'], lead_fit_ro_mess['food_blood'], lead_fit_ro_mess['lose_a_term'], lead_fit_ro_mess['activity_day'], lead_fit_ro_mess['first_name'], lead_fit_ro_mess['gender'], lead_fit_ro_mess['answer_a'], lead_fit_ro_mess['answer_u'])
    else:
...
def handle_profile_fit(user_id, age_w_h, food_blood, lose_a_term, activity_day, first_name, gender):
    ...
    set_custom_field(user_id, "9302151", f'"{response}"')
...
def handle_offer(user_id, age_w_h, food_blood, lose_a_term, activity_day, first_name, gender):
    ...
    set_custom_field(user_id, "9302154", f'"{response}"')
    upsert_lead(user_id, answer_a=response)
    return response

def handle_answer_question(user_id, message, age_w_h, food_blood, lose_a_term, activity_day, first_name, gender, answer_a, answer_u):
    upsert_lead(user_id, answer_u=message)
    response_analys = generate_response_with_variables_gpt4(ANSWER_QUESTION_ANALYSIS, ROLE_ANSWER_QUESTION_ANALYSIS, max_tokens=100, temperature=0.9, top_p=0.7, age_w_h=age_w_h, food_blood=food_blood, lose_a_term=lose_a_term, activity_day=activity_day, first_name=first_name, gender=gender, answer_a=answer_a, answer_u=answer_u)
    upsert_lead(user_id, tone_message=response_analys)
    if 'заинтересованность' in response_analys:
        response = generate_response_with_variables_gpt4(ANSWER_INTEREST, ROLE_ANSWER_INTEREST, max_tokens=1500, temperature=0.9, top_p=0.7, age_w_h=age_w_h, food_blood=food_blood, lose_a_term=lose_a_term, activity_day=activity_day, first_name=first_name, gender=gender, answer_a=answer_a, answer_u=answer_u)
        set_custom_field(user_id, "9302155", f'"{response}"')
        return response
        pass
    elif 'сомнения' in response_analys:
        response = generate_response_with_variables_gpt4(ANSWER_DOUBTS, ROLE_ANSWER_DOUBTS, max_tokens=1500, temperature=0.9, top_p=0.7, age_w_h=age_w_h, food_blood=food_blood, lose_a_term=lose_a_term, activity_day=activity_day, first_name=first_name, gender=gender, answer_a=answer_a, answer_u=answer_u)
        set_custom_field(user_id, "9302155", f'"{response}"')
        return response
        pass
    elif 'негативный' in response_analys:
        response = generate_response_with_variables_gpt4(ANSWER_NEGATIVE, ROLE_ANSWER_NEGATIVE, max_tokens=1500, temperature=0.9, top_p=0.7, age_w_h=age_w_h, food_blood=food_blood, lose_a_term=lose_a_term, activity_day=activity_day, first_name=first_name, gender=gender, answer_a=answer_a, answer_u=answer_u)
        set_custom_field(user_id, "9302155", f'"{response}"')
        return response
        pass
    elif 'другие вопросы' in response_analys:
        response = generate_response_with_variables_gpt4(ANSWER_OTHER, ROLE_ANSWER_OTHER, max_tokens=1500, temperature=0.9, top_p=0.7, age_w_h=age_w_h, food_blood=food_blood, lose_a_term=lose_a_term, activity_day=activity_day, first_name=first_name, gender=gender, answer_a=answer_a, answer_u=answer_u)
        set_custom_field(user_id, "9302155", f'"{response}"')
        return response
        pass
